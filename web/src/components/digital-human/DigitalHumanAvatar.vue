<!--
  数字人展示组件
  加载后台启用的数字人配置，启动讯飞交互式虚拟人流，并对外暴露 speak() 供对话调用。
  当凭证缺失 / 连接失败 / 服务为纯文本模式时，降级为静态形象 + 文本提示。
-->
<template>
...
<script setup lang="ts">
  ...
  const fallbackTip = computed(() => {
    ...
    if (avatar.status.value === 'error') return avatar.errorMessage.value || '数字人不可用，已切换为文本模式'
  ...
  async function speak(text: string, emotion?: string): Promise<boolean> {
    const ok = await avatar.speak(text, { emotion })
    return ok
  }
